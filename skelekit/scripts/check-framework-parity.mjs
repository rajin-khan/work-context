...
const referencePackage = JSON.parse(
  fs.readFileSync(
    path.join(referenceDir, 'skelementor-utilities.skele'),
    'utf8'
  )
);
...
const buildReadableId = (
  ...
  for (
    let suffixLength = COLLISION_SUFFIX_LENGTH;
    suffixLength <= 12;
    suffixLength += 1
  ) {
...
const buildCanonicalClassId = (label, reservedIds = []) => {
...
const buildCanonicalVariableId = (cssName, reservedIds = []) =>
  buildReadableId(
...
const matchResponsiveLabel = (label) => {
...
const prioritizeResponsiveClassOrder = (order = [], items = {}) => {
  ...
  order.forEach((id, index) => {
    ...
    familyMembers[family.base].push({ id, rank: family.rank, index });
  });
...
const detectVariableType = (label, value) => {
...
const extractRootVariablesFromCss = (cssContent) => {
  const rootVariables = {};
...
const buildVariableSnapshotFromRootVariables = (rootVariables = {}) => {
  const items = {};
  const order = [];
  const reservedIds = [];
...
const mapMediaQueryToBreakpoint = (query = '') => {
  ...
  if (normalizedQuery.includes('max-width: 767px')) return 'mobile';
  if (normalizedQuery.includes('max-width: 880px')) return 'mobile_extra';
  if (normalizedQuery.includes('max-width: 1024px')) return 'tablet';
  if (normalizedQuery.includes('max-width: 1200px')) return 'tablet_extra';
  if (normalizedQuery.includes('max-width: 1366px')) return 'laptop';
  if (normalizedQuery.includes('min-width: 2400px')) return 'widescreen';
...
const buildClassSnapshotFromCss = (
  cssContent,
...
const buildCanonicalSnapshotsFromCss = (cssContent) => {
  const variableSnapshot = buildVariableSnapshotFromRootVariables(
    extractRootVariablesFromCss(cssContent)
  );
...
const assertMatch = (label, left, right) => {
  const leftJson = stableSerialize(left);
  const rightJson = stableSerialize(right);
  ...
  if (leftJson !== rightJson) {
    ...
    if (
      ...
      const orderMismatchIndex = leftOrder.findIndex(
        (value, index) => value !== rightOrder[index]
      );
      if (orderMismatchIndex !== -1) {
        console.error(
          `First order mismatch at index ${orderMismatchIndex}:`,
          leftOrder[orderMismatchIndex],
          rightOrder[orderMismatchIndex]
        );
      }

      const mismatchedId =
        leftOrder.find(
          (id) => stableSerialize(left.items[id]) !== stableSerialize(right.items[id])
        ) ||
        Object.keys(left.items).find(
          (id) =>
            stableSerialize(left.items[id]) !== stableSerialize(right.items[id])
        );

      if (mismatchedId) {
        console.error('First mismatched item:', mismatchedId);
        console.error('Generated:', JSON.stringify(left.items[mismatchedId], null, 2));
        console.error('Reference:', JSON.stringify(right.items[mismatchedId], null, 2));
      }
    }
    process.exitCode = 1;
    return;
...
assertMatch(
  'Package format',
  {
    format: referencePackage.format,
...
console.log('Framework parity checks passed.');
