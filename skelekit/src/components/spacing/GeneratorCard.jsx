...
import Select from '../ui/Select'; // Import our Select component

const GeneratorCard = ({ id, className, properties, enabled, onUpdate, onRemove, spacingGroups, scaleGroupId }) => {
  ...
  const scaleOptions = spacingGroups.map(g => ({ label: g.name, value: g.id }));
  ...
  return (
    <motion.div 
      ...
      className="relative bg-white border border-neutral-300 rounded-lg flex transition-colors group"
    >
      <button 
        onClick={onRemove}
        className="absolute top-3 right-3 p-1.5 text-neutral-500 rounded-full hover:bg-neutral-100 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all z-10"
      ...
      <div className="w-1/3 p-6 flex flex-col justify-center gap-4">
        ...
        <div>
          <div className="relative font-mono text-xl font-semibold text-neutral-800 flex items-center bg-neutral-100 border border-transparent rounded focus-within:border-brand">
            <span className="pl-3 opacity-50 select-none pointer-events-none">.</span>
            <input type="text" value={editableName} onChange={handleClassNameChange} onBlur={handleClassNameBlur} className="bg-transparent focus:outline-none py-1 flex-1 min-w-0 text-neutral-800" style={{ paddingLeft: '1px', paddingRight: '2px' }}/>
            <span className="pr-3 opacity-50 select-none pointer-events-none">-*</span>
          </div>
          <p className="text-sm text-neutral-600 mt-1 pl-1">{enabled ? 'Enabled' : 'Disabled'}</p>
        </div>
      </div>

      {/* Column 2: Properties */}
      <div className="flex-1 p-6 border-l border-neutral-200">
        <h4 className="text-sm font-medium text-neutral-600 mb-3">Generates Properties</h4>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={handleAddProperty} className="w-8 h-8 flex items-center justify-center bg-neutral-200 text-neutral-600 rounded-full hover:bg-brand hover:text-white transition-all">
            <Plus size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="w-48 p-6 border-l border-neutral-200 flex flex-col justify-center">
        <h4 className="text-sm font-medium text-neutral-600 mb-2">Scale Source</h4>
        <Select 
